<script lang="ts">
  ...
  // Auto-detect: if the queue exactly matches a saved playlist and no playlist is selected yet,
  // show its name without requiring the user to explicitly load it every session.
  $effect(() => {
    if (selectedPlaylistId || !playlists.loaded || playlists.mine.length === 0 || tracks.length === 0) return
    const setIds = new Set(tracks.map((t) => t.videoId))
    const match = playlists.mine.find(
      (pl) => pl.tracks.length === setIds.size && pl.tracks.every((t) => setIds.has(t.videoId)),
    )
    if (match) selectedPlaylistId = match.id
  })
