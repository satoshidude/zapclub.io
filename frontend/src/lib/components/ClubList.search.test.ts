...
describe('club search suggestions', () => {
  ...
  it('renders live clubs with the player information hierarchy and no player controls', () => {
    ...
    expect(source).toMatch(/\.pic-img\s*\{[\s\S]*?height: 100%;[\s\S]*?border-radius: 0;[\s\S]*?object-fit: cover;/)
    expect(source).toMatch(/\.search-suggestion img\s*\{[\s\S]*?border-radius: 0;/)
