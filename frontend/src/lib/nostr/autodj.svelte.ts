...
export async function armAutoDJ(clubId: string, playlist: Playlist): Promise<void> {
  ...
  // Step off this club's stage so the conductor's auto-DJ path takes over cleanly
  // without the owner appearing in the round-robin. Only affects this specific club —
  // if the owner is on stage in a different club, that stage is left untouched.
  if (persistedStageGroup() === clubId) {
    await leaveStage(clubId)
  }
