...
describe('player DJ zap action', () => {
  ...
  it('renders a dedicated collapsed-player action with the DJ name centered there', () => {
    ...
    expect(nowPlaying).toMatch(/\.zap-stage-action :global\(\.bolt-icon\) \{[\s\S]*?grid-row: 2;[\s\S]*?justify-self: center;/)
    expect(nowPlaying).toMatch(/\.zap-stage-action :global\(\.icon-dj-name\) \{[\s\S]*?grid-row: 1;[\s\S]*?justify-self: center;/)
    ...
    expect(nowPlaying).toContain('animation: zap-icon-turn 7s linear infinite')
    expect(nowPlaying).toContain('transform: perspective(240px) rotateY(360deg)')
    expect(nowPlaying).not.toMatch(/\.zap-stage-action \{[^}]*perspective:/)
    expect(nowPlaying).toContain('class="zap-sparkles" aria-hidden="true"')
    expect(nowPlaying.match(/class="zap-star"/g)).toHaveLength(4)
    expect(nowPlaying).toContain('animation: zap-star-flash 6.4s ease-in-out infinite')
    expect(nowPlaying).not.toContain('.zap-stage-action::before')
    expect(nowPlaying).not.toContain('conic-gradient')
