 ...
 *  Relay-authored updates (CLUB_RELAY_PUBKEY) carry a "p" tag naming the actual DJ; those are
 *  indexed under the DJ's pubkey so offline DJ queues drain correctly in "Up next". */
export function ingestQueue(ev: Event): void {
  // Relay-authored queue updates use a "p" tag to identify the actual DJ.
  let djPk = ev.pubkey
  if (ev.pubkey === CLUB_RELAY_PUBKEY) {
    const pTag = ev.tags.find((t) => t[0] === 'p')?.[1]
    if (!pTag) return // relay-authored queue without "p" tag — not a managed queue update
    djPk = pTag
  }
  const prev = state.byDj[djPk]
  ...
  console.log(`[zc:queue] ingest: ${djPk.slice(0, 8)} tracks=${tracks.length} active=${tracks.filter((t) => t.active !== false).length}`)
}
...
/**
 * Reactivate ALL of my tracks for a club (clear the `off`/played flags). Only fires when the
 * queue is FULLY exhausted (every track is off) — a returning DJ with a partially-played set
 * continues from where they left off rather than replaying already-played tracks.
 * No-op if any tracks are still active, if nothing is off, or if not signed in.
 */
export async function reactivateMyQueue(groupId: string): Promise<void> {
  ...
  // Only reactivate when the queue is completely exhausted — partial off-flags mean the DJ is
  // mid-set and those marks must be preserved so already-played tracks don't replay.
  if (!tracks.every((t) => t.active === false)) return
