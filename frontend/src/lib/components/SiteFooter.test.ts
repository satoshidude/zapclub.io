...
describe('project footer contract', () => {
  ...
  it('links TOP 10, About, Disclaimer and GitHub from one footer menu', () => {
    expect(source).toContain('<nav class="footer-nav"')
    expect(source.match(/<a\b/g)).toHaveLength(5)
    expect(source).toContain('<a href="/leaderboard" onclick={(event) => follow(event, \'/leaderboard\')}>TOP 10</a>')
