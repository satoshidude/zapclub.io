...
import { KIND_STAGE, publishSessionClub, type ClubOperationIntent } from './groups'
import { auth } from './auth.svelte'
import { selectActiveDjs, MAX_DJS } from './conductor'
import type { StageDj } from './types'
import { startPresence, stopPresence } from './presence.svelte'
...
function cacheStage(groupId: string): void {
  try {
    ...
    for (const [pk, entry] of Object.entries(state.djs)) {
      // Never persist provisional (seeded) entries — a ghost written to the cache would
      // re-seed itself with a fresh lastSeen on every load and become immortal.
      if (entry.on && !entry.seeded) snapshot[pk] = { since: entry.since, lastSeen: entry.lastSeen, on: entry.on }
    }
 ...
 *  All entries are provisional (`seeded`): any relay event overwrites them, and
 *  pruneSeeded() removes whatever the relay did not confirm by EOSE. */
export function seedStageFromCache(groupId: string): void {
  try {
    const raw = localStorage.getItem(STAGE_CACHE_PREFIX + groupId)
    const snapshot = raw
      ? (JSON.parse(raw) as Record<string, { since: number; lastSeen: number; on: boolean }>)
      : {}
    let seeded = 0
    for (const [pk, entry] of Object.entries(snapshot)) {
      if (!state.djs[pk]) {
        state.djs[pk] = { since: Number(entry.since), lastSeen: Number(entry.lastSeen), on: Boolean(entry.on), seeded: true }
        seeded++
      }
    }
    // Own on-stage marker: only seed ourselves for the club the marker belongs to —
    // the stage state is global, so seeding without the club check painted the user
    // onto EVERY club's stage after a reload.
    const me = auth.pubkey
    if (me && !state.djs[me]) {
      const since = persistedSince(groupId)
      if (since !== null) {
        state.djs[me] = { since, lastSeen: Date.now(), on: true, seeded: true }
        seeded++
      }
    }
    console.log(`[zc:stage] seedFromCache: ${seeded} djs seeded (${Object.keys(snapshot).length} cached)`)
  } catch {
    /* ignore */
  }
}
/** Drop entries the relay did not confirm by EOSE — seeds exist only to bridge the
 *  gap between mount and the 30102 backfill. */
export function pruneSeeded(): void {
  for (const [pk, entry] of Object.entries(state.djs)) {
    if (entry.seeded) {
      console.log(`[zc:stage] pruneSeeded: drop unconfirmed ${pk8(pk)}`)
      delete state.djs[pk]
    }
  }
}
...
// Heartbeat every 2 min — the 5-min stage window (STALE_MS in conductor.ts, relay
// condStageStaleMS) tolerates one missed beat. Join/leave are posted immediately, plus an
// instant refresh when the tab returns.
const HEARTBEAT_MS = 120_000

interface StageState {
  /** pubkey → last state. `seeded` = provisional (from localStorage, not relay-confirmed). */
  djs: Record<string, { since: number; lastSeen: number; on: boolean; eventId?: string; seeded?: boolean }>
...
// NOTE: the current user's own stage entry is seeded club-aware in seedStageFromCache()
// (marker group must match the club being viewed). A global module-load seed painted the
// user onto EVERY club's stage after a reload — with the marker pointing at club A, the
// ghost appeared on club B's stage, was skipped-over by B's older relay events, written
// into B's stage cache, and the round-robin preview showed the ghost's tracks as "up
// next" forever without the conductor ever playing them.
...
/**
 * Active stage DJs: on + fresh (<5 min) + not kicked (last on-heartbeat newer than the last
...
export function ingestStage(ev: Event): void {
  ...
  const prev = state.djs[principal]
  // Only consider the newest event per DJ. Relay events ALWAYS beat provisional seeds —
  // a seed carries lastSeen=now, so an age comparison would let a localStorage ghost
  // outrank the relay's actual (older) state forever.
  const losesCanonicalTie = !!prev?.eventId && lastSeen === prev.lastSeen && ev.id >= prev.eventId
  if (prev && !prev.seeded && (lastSeen < prev.lastSeen || losesCanonicalTie)) {
    console.log(`[zc:stage] ingestStage: skip old ${pk8(principal)} on=${on}`)
    return
  }
  ...
  state.djs[principal] = { since, lastSeen, on, eventId: ev.id }
  const onCount = Object.values(state.djs).filter((d) => d.on).length
  console.log(`[zc:stage] ingestStage: ${pk8(principal)} on=${on} since=${since} total_on=${onCount}`)
...
function postStageAutomatically(groupId: string, on: boolean): void {
  void postStage(groupId, on, 'automatic').catch(() => {})
}

/** Go on stage: post on-stage + start the heartbeat. */
export async function joinStage(
  groupId: string,
  intent: ClubOperationIntent = 'explicit',
): Promise<void> {
  ...
  if (myGroupId && myGroupId !== groupId) postStageAutomatically(myGroupId, false)
  const prevGroup = myGroupId
  myGroupId = groupId
  ...
  try {
    await postStage(groupId, true, intent)
  } catch (e) {
    // Relay refused the join (e.g. "restricted: stage is full" from the DJ-cap gate).
    // Roll back so the client doesn't ghost itself on stage: the persisted marker would
    // otherwise resurrect a local on-stage entry (seedMyStage) that the relay's older
    // off-event can never overwrite — the round-robin preview then shows this DJ's
    // tracks as "up next" forever without the conductor ever playing them.
    myGroupId = prevGroup === groupId ? null : prevGroup
    forgetStage()
    const me = auth.pubkey
    if (me && state.djs[me]?.on) {
      state.djs[me] = { ...state.djs[me], on: false, lastSeen: Date.now() }
    }
    console.log(`[zc:stage] joinStage: rejected — rolled back (${String((e as Error)?.message ?? e)})`)
    throw e
  }
  ensureTicking()
  ...
  hbTimer = setInterval(() => {
    if (myGroupId) {
      ...
      postStageAutomatically(myGroupId, true)
    }
  }, HEARTBEAT_MS)
  // A sticky stage owns a Presence claim even when ClubView is gone. The shared scheduler
  // deduplicates it with the view claim while both point at the same club.
  startPresence(groupId, 'stage')
}
...
export async function leaveStage(
  ...
  if (gid) await postStage(gid, false, intent)
}
...
export function resetStage(): void {
  ...
  if (myGroupId) postStageAutomatically(myGroupId, false)
  myGroupId = null
  state.djs = {}
  state.kicks = {}
}
...
if (typeof document !== 'undefined') {
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
      state.tick = Date.now()
      if (myGroupId) postStageAutomatically(myGroupId, true)
    }
  })
}
